...
1  let H = min heap data structure, initialized with 0 and s [here 0 indicates the distance from start vertex s]
2  while H is non-empty:
3    remove the first node and cost of H, call it U and cost
4    if U has been previously explored:
5       go to the while loop, line 2 //Once a node is explored there is no need to make it again
